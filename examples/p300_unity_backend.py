...
test_erp.classifier.set_p300_clf_settings(n_splits=5, oversample_ratio=0, undersample_ratio=0, covariance_estimator='oas')
# initial_subset = []
...
#test_erp.stream_online_eeg_data(subset = ['P3', 'C3', 'F3', 'Fz', 'F4', 'C4', 'P4', 'Pz', 'Fp1', 'Fp2', 'T3', 'T5', 'O1', 'O2', 'T6', 'T4']) # you can also add a subset here
# Run main
# test_erp.main(online=True, training=True, pp_low=0.1, pp_high=15, pp_order=5, plot_erp=False, window_start=0.0, window_end=0.6)
test_erp.main(online=True, training=True, pp_low=0.1, pp_high=10, pp_order=5, plot_erp=False, window_start=0.0, window_end=0.8, print_markers=True, print_training=True, print_fit=True, print_performance=True, print_predict=True,max_decisions=100)